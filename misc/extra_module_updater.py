...
async def unified_update_handler(bot: BOT, message: Message):
    ...
    if "-repo" in message.flags:
        await message.reply(
            ...
            f"<pre language=Note><i>If you've forked the repository and the official repo data is still showing up here, it means you haven't changed the repository data in the '<code>settings.py</code>' file.\n\nChange <code>REPO_OWNER</code> and <code>REPO_NAME</code> to your own if you want the script to check for updates to your repo.</i></pre>"
        )
        return
